...
static PyMemAllocatorEx C89_MALLOC_ALLOC = MALLOC_ALLOC;
#ifdef WITH_PYMALLOC
static PyMemAllocatorEx C89_OBJ_ALLOC = PYMALLOC_ALLOC;
static PyMemAllocatorEx C89_MEM_ALLOC = PYMALLOC_ALLOC;
#else
static PyMemAllocatorEx C89_OBJ_ALLOC = MALLOC_ALLOC;
static PyMemAllocatorEx C89_MEM_ALLOC = MALLOC_ALLOC;
#endif
#endif
                            ...
                            PyMemAllocatorEx *old_alloc)
{
  ...
  { PyMemAllocatorEx new_alloc;  /*C89 -- mixed declarations and code*/
    switch(domain)
    {
    case PYMEM_DOMAIN_RAW:
        new_alloc = /*(PyMemAllocatorEx)PYRAW_ALLOC*/C89_MALLOC_ALLOC;
        break;
    case PYMEM_DOMAIN_MEM:
        new_alloc = /*(PyMemAllocatorEx)PYMEM_ALLOC*/C89_MEM_ALLOC;
        break;
    case PYMEM_DOMAIN_OBJ:
        new_alloc = /*(PyMemAllocatorEx)PYOBJ_ALLOC*/C89_OBJ_ALLOC;
        break;
    default:
        /* unknown domain */
        return -1;
...
_PyMem_SetupAllocators(const char *opt)
{
    ...
    if (strcmp(opt, "default") == 0) {
      ...
      { PyMemAllocatorEx pymalloc = PYMALLOC_ALLOC;  /*C89 -- mixed declarations and code*/
...
_PyMem_GetAllocatorsName(void)
{
  ...
  { PyMemAllocatorEx dbg_raw = PYDBGRAW_ALLOC;  /*C89 -- mixed declarations and code*/
...
_PyMem_RawWcsdup(const wchar_t *str)
{
  ...
  { size_t len = wcslen(str);  /*C89 -- mixed declarations and code*/
...
{
    assert(str != NULL);
  { size_t size = strlen(str) + 1;  /*C89 -- mixed declarations and code*/
...
_PyObject_Calloc(void *ctx, size_t nelem, size_t elsize)
{
  ...
  { size_t nbytes = nelem * elsize;  /*C89 -- mixed declarations and code*/
...
_PyMem_IsFreed(void *ptr, size_t size)
{
	...
	size_t i;
    for (/*size_t*/ i=0; i < size; i++) {  /*C89 -- mixed declarations and code*/
        if (bytes[i] != DEADBYTE) {
            return 0;
        }
    }
...
_PyMem_DebugRawFree(void *ctx, void *p)
{
  ...
  { debug_alloc_api_t *api = (debug_alloc_api_t *)ctx;  /*C89 -- mixed declarations and code*/
...
_PyObject_DebugMallocStats(FILE *out)
{
  ...
  { uint i;  /*C89 -- mixed declarations and code*/
