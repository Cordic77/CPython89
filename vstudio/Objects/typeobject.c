...
lookup_maybe_method(PyObject *self, _Py_Identifier *attrid, int *unbound)
{
    ...
    if (PyFunction_Check(res)) {
        ...
        *unbound = 0;
      { descrgetfunc f = Py_TYPE(res)->tp_descr_get;  /*C89 -- mixed declarations and code*/
        if (f == NULL) {
            Py_INCREF(res);
        }
        else {
...
type_getattro(PyTypeObject *type, PyObject *name)
{
    ...
    if (attribute != NULL) {
        ...
        Py_INCREF(attribute);
      { descrgetfunc local_get = Py_TYPE(attribute)->tp_descr_get;  /*C89 -- mixed declarations and code*/
...
slot_tp_richcompare(PyObject *self, PyObject *other, int op)
{
  ...
  { PyObject *args[1] = {other};  /*C89 -- mixed declarations and code*/
    res = call_unbound(unbound, func, self, args, 1);
  }
    Py_DECREF(func);
    return res;
