...
clone_combined_dict(PyDictObject *orig)
{
  ...
  { Py_ssize_t keys_size = _PyDict_KeysSize(orig->ma_keys);  /*C89 -- mixed declarations and code*/
  ...
  { PyDictKeyEntry *ep0 = DK_ENTRIES(keys);
    ...
    Py_ssize_t i;
    for (/*Py_ssize_t*/ i = 0; i < n; i++) {  /*C89 -- mixed declarations and code*/
    ...
    for (;;) {
        ...
        if (ix >= 0) {
            ...
            if (ep->me_hash == hash) {
                ...
                Py_INCREF(startkey);
              { int cmp = PyObject_RichCompareBool(startkey, key, Py_EQ);  /*C89 -- mixed declarations and code*/
                Py_DECREF(startkey);
                if (cmp < 0) {
                    *value_addr = NULL;
                    return DKIX_ERROR;
                 ...
                 Py_hash_t hash, PyObject **value_addr)
{
  ...
  { PyDictKeyEntry *ep0 = DK_ENTRIES(mp->ma_keys);  /*C89 -- mixed declarations and code*/
                         ...
                         Py_hash_t hash, PyObject **value_addr)
{
  ...
  { PyDictKeyEntry *ep0 = DK_ENTRIES(mp->ma_keys);  /*C89 -- mixed declarations and code*/
    ...
    for (;;) {
      ...
      { PyDictKeyEntry *ep = &ep0[ix];  /*C89 -- mixed declarations and code*/
        assert(ep->me_key != NULL);
        assert(PyUnicode_CheckExact(ep->me_key));
        if (ep->me_key == key ||
            (ep->me_hash == hash && unicode_eq(ep->me_key, key))) {
...
find_empty_slot(PyDictKeysObject *keys, Py_hash_t hash)
{
  ...
  { const size_t mask = DK_MASK(keys);  /*C89 -- mixed declarations and code*/
    size_t i = hash & mask;
    Py_ssize_t ix = dk_get_index(keys, i);
    size_t perturb;
...
insertdict(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject *value)
{
  ...
  { Py_ssize_t ix = mp->ma_keys->dk_lookup(mp, key, hash, &old_value);  /*C89 -- mixed declarations and code*/
    if (ix == DKIX_ERROR)
        goto Fail;
    ...
    if (ix == DKIX_EMPTY) {
      ...
      { Py_ssize_t hashpos = find_empty_slot(mp->ma_keys, hash);  /*C89 -- mixed declarations and code*/
...
build_indices(PyDictKeysObject *keys, PyDictKeyEntry *ep, Py_ssize_t n)
{
    ...
    Py_ssize_t ix;
    for (/*Py_ssize_t*/ ix = 0; ix != n; ix++, ep++) {  /*C89 -- mixed declarations and code*/
...
dictresize(PyDictObject *mp, Py_ssize_t minsize)
{
    ...
    if (oldvalues != NULL) {
         ...
         */
      { Py_ssize_t i;
        for (/*Py_ssize_t*/ i = 0; i < numentries; i++) {  /*C89 -- mixed declarations and code*/
...
PyDict_SetDefault(PyObject *d, PyObject *key, PyObject *defaultobj)
{
  ...
  { Py_ssize_t ix = (mp->ma_keys->dk_lookup)(mp, key, hash, &value);  /*C89 -- mixed declarations and code*/
    if (ix == DKIX_ERROR)
        return NULL;
