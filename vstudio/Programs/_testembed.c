...
static int test_pre_initialization_api(void)
{
  ...
  { wchar_t *program = Py_DecodeLocale("./spam", NULL);  /*C89 -- mixed declarations and code*/
    if (program == NULL) {
...
static int test_bpo20891(void)
{
  ...
  { unsigned long thrd = PyThread_start_new_thread(bpo20891_thread, &lock);  /*C89 -- mixed declarations and code*/
...
dump_config_impl(void)
{
  ...
  { PyObject *json = PyImport_ImportModule("json");  /*C89 -- mixed declarations and code*/
...
static int test_init_from_config(void)
{
  ...
  { static wchar_t* argv[2] = {  /*C89 -- mixed declarations and code*/
        L"-c",
        L"pass",
    };
  ...
  { _PyInitError err = _Py_InitializeFromConfig(&config);  /*C89 -- mixed declarations and code*/
...
int main(int argc, char *argv[])
{
    if (argc > 1) {
        struct TestCase *tc;
        for (/*struct TestCase **/ tc = TestCases; tc && tc->name; tc++) {  /*C89 -- mixed declarations and code*/
