...
_hamt_node_array_validate(void *o)
{
  ...
  { PyHamtNode_Array *node = (PyHamtNode_Array*)(o);  /*C89 -- mixed declarations and code*/
...
hamt_node_bitmap_clone_without(PyHamtNode_Bitmap *o, uint32_t bit)
{
  ...
  { PyHamtNode_Bitmap *new = (PyHamtNode_Bitmap *)hamt_node_bitmap_new(
        Py_SIZE(o) - 2);  /*C89 -- mixed declarations and code*/
    if (new == NULL) {
        return NULL;
    }
    ...
    */
    ...
    if (key1_hash == key2_hash) {
      ...
      { PyHamtNode *n2 = hamt_node_assoc(
            n, shift, key1_hash, key1, val1, &added_leaf);  /*C89 -- mixed declarations and code*/
        Py_DECREF(n);
        if (n2 == NULL) {
            return NULL;
        }
                       ...
                       PyObject *key, PyObject *val, int* added_leaf)
{
    ...
    */
    ...
    if (self->b_bitmap & bit) {
      ...
      { PyObject *key_or_null = self->b_array[key_idx];  /*C89 -- mixed declarations and code*/
        ...
        if (key_or_null == NULL) {
          ...
          { PyHamtNode *sub_node = hamt_node_assoc(
                ...
                shift + 5, hash, key, val, added_leaf);  /*C89 -- mixed declarations and code*/
            if (sub_node == NULL) {
                return NULL;
            }
      ...
      { int comp_err = PyObject_RichCompareBool(key, key_or_null, Py_EQ);  /*C89 -- mixed declarations and code*/
        ...
        if (comp_err == 1) {  /* key == key_or_null */
          ...
          { PyHamtNode_Bitmap *ret = hamt_node_bitmap_clone(self);  /*C89 -- mixed declarations and code*/
            if (ret == NULL) {
                return NULL;
            }
            Py_INCREF(val);
        ...
        /* It's a new key, and it has the same index as *one* another key.
           ...
           a new Bitmap node.
        */
      { PyHamtNode *sub_node = hamt_node_new_bitmap_or_collision(  /*C89 -- mixed declarations and code*/
            shift + 5,
      ...
      { PyHamtNode_Bitmap *ret = hamt_node_bitmap_clone(self);
        ...
        *added_leaf = 1;
        ...
        if (n >= 16) {
          ...
          { Py_ssize_t i, j;
            for (i = 0, j = 0; i < HAMT_ARRAY_NODE_SIZE; i++) {  /*C89 -- mixed declarations and code*/
                         ...
                         PyHamtNode **new_node)
{
  ...
  { uint32_t idx = hamt_bitindex(self->b_bitmap, bit);  /*C89 -- mixed declarations and code*/
            ...
            case W_NEWNODE: {
                ...
                if (IS_BITMAP_NODE(sub_node)) {
                            ...
                            sub_tree->b_array[0] != NULL)
                    {
                      ...
                      { PyObject *key = sub_tree->b_array[0];  /*C89 -- mixed declarations and code*/
              ...
              { PyHamtNode_Bitmap *clone = hamt_node_bitmap_clone(self);  /*C89 -- mixed declarations and code*/
                if (clone == NULL) {
                    return W_ERROR;
                }
                               ...
                               Py_ssize_t *idx)
{
    ...
    for (i = 0; i < Py_SIZE(self); i += 2) {
        ...
        assert(el != NULL);
      { int cmp = PyObject_RichCompareBool(key, el, Py_EQ);  /*C89 -- mixed declarations and code*/
        if (cmp < 0) {
            return F_ERROR;
        }
        if (cmp == 1) {
    ...
    if (hash == self->c_hash) {
        ...
        switch (found) {
            ...
            case F_FOUND:
              ...
              { Py_ssize_t val_idx = key_idx + 1;  /*C89 -- mixed declarations and code*/
                            ...
                            PyHamtNode **new_node)
{
  ...
  { Py_ssize_t key_idx = -1;  /*C89 -- mixed declarations and code*/
        ...
        case F_FOUND:
          ...
          { PyHamtNode_Collision *new = (PyHamtNode_Collision *)
                hamt_node_collision_new(
                    self->c_hash, Py_SIZE(self) - 2);  /*C89 -- mixed declarations and code*/
            if (new == NULL) {
                return W_ERROR;
            }
                        ...
                        PyHamtNode **new_node)
{
  ...
  { PyHamtNode *sub_node = NULL;  /*C89 -- mixed declarations and code*/
    hamt_without_t res = hamt_node_without(
        (PyHamtNode *)node,
        shift + 5, hash, key, &sub_node);
    ...
    switch (res) {
        ...
        case W_NEWNODE: {
          ...
          { PyHamtNode_Array *clone = hamt_node_array_clone(self);  /*C89 -- mixed declarations and code*/
            if (clone == NULL) {
                Py_DECREF(sub_node);
                return W_ERROR;
            }
        ...
        case W_EMPTY: {
            ...
            */
          { Py_ssize_t new_count = self->a_count - 1;  /*C89 -- mixed declarations and code*/
          ...
          { Py_ssize_t new_i = 0;  /*C89 -- mixed declarations and code*/
            uint32_t i;
...
{   uint32_t i;
    for (/*uint32_t*/ i = 0; i < _Py_HAMT_MAX_TREE_DEPTH; i++) {  /*C89 -- mixed declarations and code*/
        iter->i_nodes[i] = NULL;
        iter->i_pos[i] = 0;
    }
                         ...
                         PyObject **key, PyObject **val)
{
  ...
  { Py_ssize_t i;
    for (/*Py_ssize_t*/ i = pos; i < HAMT_ARRAY_NODE_SIZE; i++) {  /*C89 -- mixed declarations and code*/
        if (node->a_array[i] != NULL) {
  ...
  { PyHamtNode *new_root;  /*C89 -- mixed declarations and code*/
    ...
    switch (res) {
        ...
        case W_NEWNODE: {
          ...
          { PyHamtObject *new_o = hamt_alloc();  /*C89 -- mixed declarations and code*/
            if (new_o == NULL) {
                Py_DECREF(new_root);
                return NULL;
            }
...
hamt_find(PyHamtObject *o, PyObject *key, PyObject **val)
{
  ...
  { int32_t key_hash = hamt_hash(key);  /*C89 -- mixed declarations and code*/
    if (key_hash == -1) {
        return F_ERROR;
    }
...
_PyHamt_Eq(PyHamtObject *v, PyHamtObject *w)
{
  ...
  { PyHamtIteratorState iter;  /*C89 -- mixed declarations and code*/
...
_PyHamt_New(void)
{
  ...
  { PyHamtObject *o = hamt_alloc();  /*C89 -- mixed declarations and code*/
    if (o == NULL) {
        return NULL;
    }
...
static inline void init_PyHamtItems_Type (void)
{
  if (_PyHamtItems_Type.tp_dealloc == NULL)
  {
    _PyHamtItems_Type.tp_basicsize = sizeof(PyHamtIterator);
    _PyHamtItems_Type.tp_itemsize = 0;
    _PyHamtItems_Type.tp_as_mapping = &PyHamtIterator_as_mapping;
    _PyHamtItems_Type.tp_dealloc = (destructor)hamt_baseiter_tp_dealloc;
    _PyHamtItems_Type.tp_getattro = PyObject_GenericGetAttr;
    _PyHamtItems_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC;
    _PyHamtItems_Type.tp_traverse = (traverseproc)hamt_baseiter_tp_traverse;
    _PyHamtItems_Type.tp_clear = (inquiry)hamt_baseiter_tp_clear;
    _PyHamtItems_Type.tp_iter = PyObject_SelfIter;
    _PyHamtItems_Type.tp_iternext = (iternextfunc)hamt_baseiter_tp_iternext;
  }
}
#endif
...
static inline void init_PyHamtKeys_Type (void)
{
  if (_PyHamtKeys_Type.tp_dealloc == NULL)
  {
    _PyHamtKeys_Type.tp_basicsize = sizeof(PyHamtIterator);
    _PyHamtKeys_Type.tp_itemsize = 0;
    _PyHamtKeys_Type.tp_as_mapping = &PyHamtIterator_as_mapping;
    _PyHamtKeys_Type.tp_dealloc = (destructor)hamt_baseiter_tp_dealloc;
    _PyHamtKeys_Type.tp_getattro = PyObject_GenericGetAttr;
    _PyHamtKeys_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC;
    _PyHamtKeys_Type.tp_traverse = (traverseproc)hamt_baseiter_tp_traverse;
    _PyHamtKeys_Type.tp_clear = (inquiry)hamt_baseiter_tp_clear;
    _PyHamtKeys_Type.tp_iter = PyObject_SelfIter;
    _PyHamtKeys_Type.tp_iternext = (iternextfunc)hamt_baseiter_tp_iternext;
  }
}
#endif
...
static inline void init_PyHamtValues_Type (void)
{
  if (_PyHamtValues_Type.tp_dealloc == NULL)
  {
    _PyHamtValues_Type.tp_basicsize = sizeof(PyHamtIterator);
    _PyHamtValues_Type.tp_itemsize = 0;
    _PyHamtValues_Type.tp_as_mapping = &PyHamtIterator_as_mapping;
    _PyHamtValues_Type.tp_dealloc = (destructor)hamt_baseiter_tp_dealloc;
    _PyHamtValues_Type.tp_getattro = PyObject_GenericGetAttr;
    _PyHamtValues_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC;
    _PyHamtValues_Type.tp_traverse = (traverseproc)hamt_baseiter_tp_traverse;
    _PyHamtValues_Type.tp_clear = (inquiry)hamt_baseiter_tp_clear;
    _PyHamtValues_Type.tp_iter = PyObject_SelfIter;
    _PyHamtValues_Type.tp_iternext = (iternextfunc)hamt_baseiter_tp_iternext;
  }
}
#endif
...
hamt_tp_richcompare(PyObject *v, PyObject *w, int op)
{
  ...
  { int res = _PyHamt_Eq((PyHamtObject *)v, (PyHamtObject *)w);  /*C89 -- mixed declarations and code*/
    if (res < 0) {
        return NULL;
    }
...
static inline void init_PyHamt_Type (void)
{
  if (_PyHamt_Type.tp_dealloc == NULL)
  {
    _PyHamt_Type.tp_methods = PyHamt_methods;
    _PyHamt_Type.tp_as_mapping = &PyHamt_as_mapping;
    _PyHamt_Type.tp_as_sequence = &PyHamt_as_sequence;
    _PyHamt_Type.tp_iter = (getiterfunc)hamt_tp_iter;
    _PyHamt_Type.tp_dealloc = (destructor)hamt_tp_dealloc;
    _PyHamt_Type.tp_getattro = PyObject_GenericGetAttr;
    _PyHamt_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC;
    _PyHamt_Type.tp_richcompare = hamt_tp_richcompare;
    _PyHamt_Type.tp_traverse = (traverseproc)hamt_tp_traverse;
    _PyHamt_Type.tp_clear = (inquiry)hamt_tp_clear;
    _PyHamt_Type.tp_new = hamt_tp_new;
    _PyHamt_Type.tp_weaklistoffset = offsetof(PyHamtObject, h_weakreflist);
    _PyHamt_Type.tp_hash = PyObject_HashNotImplemented;
  }
}
#endif
...
static inline void init_PyHamt_ArrayNode_Type (void)
{
  if (_PyHamt_ArrayNode_Type.tp_dealloc == NULL)
  {
    _PyHamt_ArrayNode_Type.tp_dealloc = (destructor)hamt_node_array_dealloc;
    _PyHamt_ArrayNode_Type.tp_getattro = PyObject_GenericGetAttr;
    _PyHamt_ArrayNode_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC;
    _PyHamt_ArrayNode_Type.tp_traverse = (traverseproc)hamt_node_array_traverse;
    _PyHamt_ArrayNode_Type.tp_free = PyObject_GC_Del;
    _PyHamt_ArrayNode_Type.tp_hash = PyObject_HashNotImplemented;
  }
}
#endif
...
static inline void init_PyHamt_BitmapNode_Type (void)
{
  if (_PyHamt_BitmapNode_Type.tp_dealloc == NULL)
  {
    _PyHamt_BitmapNode_Type.tp_dealloc = (destructor)hamt_node_bitmap_dealloc;
    _PyHamt_BitmapNode_Type.tp_getattro = PyObject_GenericGetAttr;
    _PyHamt_BitmapNode_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC;
    _PyHamt_BitmapNode_Type.tp_traverse = (traverseproc)hamt_node_bitmap_traverse;
    _PyHamt_BitmapNode_Type.tp_free = PyObject_GC_Del;
    _PyHamt_BitmapNode_Type.tp_hash = PyObject_HashNotImplemented;
  }
}
#endif
...
static inline void init_PyHamt_CollisionNode_Type (void)
{
  if (_PyHamt_CollisionNode_Type.tp_dealloc == NULL)
  {
    _PyHamt_CollisionNode_Type.tp_dealloc = (destructor)hamt_node_collision_dealloc;
    _PyHamt_CollisionNode_Type.tp_getattro = PyObject_GenericGetAttr;
    _PyHamt_CollisionNode_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC;
    _PyHamt_CollisionNode_Type.tp_traverse = (traverseproc)hamt_node_collision_traverse;
    _PyHamt_CollisionNode_Type.tp_free = PyObject_GC_Del;
    _PyHamt_CollisionNode_Type.tp_hash = PyObject_HashNotImplemented;
  }
}
#endif
...
_PyHamt_Init(void)
{
    ...
    init_PyHamt_Type();
    init_PyHamt_ArrayNode_Type();
    init_PyHamt_BitmapNode_Type();
    init_PyHamt_CollisionNode_Type();
    init_PyHamtKeys_Type();
    init_PyHamtValues_Type();
    init_PyHamtItems_Type();
    #endif
