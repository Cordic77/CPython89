...
Py_SetStandardStreamEncoding(const char *encoding, const char *errors)
{
  ...
  { int res = 0;  /*C89 -- mixed declarations and code*/
                        ...
                        const _PyCoreConfig *core_config)
{
  ...
  { PyThreadState *tstate = PyThreadState_New(interp);  /*C89 -- mixed declarations and code*/
    if (tstate == NULL)
  ...
  { PyObject *modules = PyDict_New();  /*C89 -- mixed declarations and code*/
    if (modules == NULL)
...
_Py_InitializeFromConfig(const _PyCoreConfig *config)
{
  ...
  { PyInterpreterState *interp;  /*C89 -- mixed declarations and code*/
    _PyInitError err;
...
Py_InitializeEx(int install_sigs)
{
  ...
  { _PyInitError err;  /*C89 -- mixed declarations and code*/
    _PyCoreConfig config = _PyCoreConfig_INIT;
...
Py_FinalizeEx(void)
{
  ...
  { /*C89 -- mixed declarations and code*/
#ifdef Py_REF_DEBUG
...
new_interpreter(PyThreadState **tstate_p)
{
  ...
  { _PyCoreConfig *core_config;  /*C89 -- mixed declarations and code*/
...
/* dup() is faster than fstat(): fstat() can require input/output operations,
  ...
  { int fd2;  /*C89 -- mixed declarations and code*/
...
fatal_error(const char *prefix, const char *msg, int status)
{
    ...
    /* Check if the current thread has a Python thread state
       and holds the GIL */
  { PyThreadState *tss_tstate = PyGILState_GetThisThreadState();  /*C89 -- mixed declarations and code*/
    if (tss_tstate != NULL) {
        PyThreadState *tstate = PyThreadState_GET();
