...
compiler_jump_if(struct compiler *c, expr_ty e, basicblock *next, int cond)
{
    ...
    case BoolOp_kind: {
        ...
        assert(n >= 0);
      { int cond2 = e->v.BoolOp.op == Or;  /*C89 -- mixed declarations and code*/
    ...
    case Compare_kind: {
        ...
        if (n > 0) {
          ...
          { basicblock *end = compiler_new_block(c);  /*C89 -- mixed declarations and code*/
            if (end == NULL)
                return 0;
            ADDOP_JREL(c, JUMP_FORWARD, end);
...
compiler_async_for(struct compiler *c, stmt_ty s)
{
  ...
  { PyObject *stop_aiter_error = _PyUnicode_FromId(&PyId_StopAsyncIteration);  /*C89 -- mixed declarations and code*/
...
stackdepth(struct compiler *c)
{
    ...
    while (sp != stack) {
        b = *--sp;
      { int depth = b->b_startdepth;  /*C89 -- mixed declarations and code*/
        assert(depth >= 0);
      { basicblock *next = b->b_next;
        ...
        for (/*int*/ i = 0; i < b->b_iused; i++) {
          ...
          { int new_depth = depth + effect;  /*C89 -- mixed declarations and code*/
            ...
            if (instr->i_jrel || instr->i_jabs) {
              ...
              { int target_depth = depth + effect;  /*C89 -- mixed declarations and code*/
