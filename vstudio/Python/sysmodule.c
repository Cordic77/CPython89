...
sys_breakpointhook(PyObject *self, PyObject *const *args, Py_ssize_t nargs, PyObject *keywords)
{
  ...
  { char *envar = Py_GETENV("PYTHONBREAKPOINT");  /*C89 -- mixed declarations and code*/
...
sys_setcheckinterval(PyObject *self, PyObject *args)
{
    if (PyErr_WarnEx(PyExc_DeprecationWarning,
                     ...
                     "instead.", 1) < 0)
        return NULL;
  { PyInterpreterState *interp = PyThreadState_GET()->interp;  /*C89 -- mixed declarations and code*/
...
sys_get_coroutine_wrapper(PyObject *self, PyObject *args)
{
  ...
  { PyObject *wrapper = _PyEval_GetCoroutineWrapper();  /*C89 -- mixed declarations and code*/
    if (wrapper == NULL) {
        wrapper = Py_None;
    }
    Py_INCREF(wrapper);
...
_append_preinit_entry(_Py_PreInitEntry *optionlist, const wchar_t *value)
{
     ...
     */
  { _Py_PreInitEntry last_entry = *optionlist;  /*C89 -- mixed declarations and code*/
    if (last_entry == NULL) {
...
_PySys_ReadPreInitOptions(void)
{
  ...
  { _Py_PreInitEntry entry = _preinit_warnoptions;  /*C89 -- mixed declarations and code*/
    while (entry != NULL) {
...
PySys_ResetWarnOptions(void)
{
  ...
  { PyObject *warnoptions = _PySys_GetObjectId(&PyId_warnoptions);  /*C89 -- mixed declarations and code*/
    if (warnoptions == NULL || !PyList_Check(warnoptions))
        return;
...
PySys_AddWarnOption(const wchar_t *s)
{
  ...
  { PyObject *unicode;  /*C89 -- mixed declarations and code*/
...
_PySys_AddXOptionWithError(const wchar_t *s)
{
  ...
  { const wchar_t *name_end = wcschr(s, L'=');  /*C89 -- mixed declarations and code*/
    if (!name_end) {
...
#ifdef MS_WINDOWS
  { const wchar_t *baseExecutable = _wgetenv(L"__PYVENV_BASE_EXECUTABLE__");  /*C89 -- mixed declarations and code*/
    if (baseExecutable) {
...
PySys_SetArgvEx(int argc, wchar_t **argv, int updatepath)
{
    ...
    if (updatepath) {
      ...
      { PyObject *sys_path = _PySys_GetObjectId(&PyId_path);  /*C89 -- mixed declarations and code*/
        if (sys_path != NULL) {
