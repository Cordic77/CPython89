...
new_identifier(const char *n, struct compiling *c)
{
    ...
    if (!PyUnicode_IS_ASCII(id)) {
      ...
      { PyObject *form = _PyUnicode_FromId(&PyId_NFKC);  /*C89 -- mixed declarations and code*/
        if (form == NULL) {
            Py_DECREF(id);
            return NULL;
        }
                            ...
                            size_t len)
{
  ...
  { const char *first_invalid_escape;  /*C89 -- mixed declarations and code*/
...
static void fstring_shift_node_locations(node *n, int lineno, int col_offset)
{
  ...
  { int i;
    for (/*int*/ i = 0; i < NCH(n); ++i) {  /*C89 -- mixed declarations and code*/
