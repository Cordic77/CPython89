...
tb_next_set(PyTracebackObject *self, PyObject *new_next, void *Py_UNUSED(_))
{
    ...
    /* Check for loops */
  { PyTracebackObject *cursor = (PyTracebackObject *)new_next;  /*C89 -- mixed declarations and code*/
    while (cursor) {
        if (cursor == self) {
  ...
  { PyObject *old_next = (PyObject*)self->tb_next;  /*C89 -- mixed declarations and code*/
    Py_XINCREF(new_next);
...
tb_print_line_repeated(PyObject *f, long cnt)
{
  ...
  { PyObject *line = PyUnicode_FromFormat(
        ...
        cnt);  /*C89 -- mixed declarations and code*/
    if (line == NULL) {
        return -1;
    }
