...
PyThread_start_new_thread(void (*func)(void *), void *arg)
{
    ...
    obj->func = func;
    obj->arg = arg;
  { PyThreadState *tstate = PyThreadState_GET();  /*C89 -- mixed declarations and code*/
...
PyThread_tss_create(Py_tss_t *key)
{
  ...
  { DWORD result = TlsAlloc();  /*C89 -- mixed declarations and code*/
    if (result == TLS_OUT_OF_INDEXES) {
        return -1;
    }
...
{
    assert(key != NULL);
  { BOOL ok = TlsSetValue(key->_key, value);  /*C89 -- mixed declarations and code*/
    return ok ? 0 : -1;
  }
}
...
PyThread_tss_get(Py_tss_t *key)
{
     ...
     * do it here.
     */
  { DWORD error = GetLastError();  /*C89 -- mixed declarations and code*/
