...
fold_unaryop(expr_ty node, PyArena *arena, int optimize)
{
  ...
  { typedef PyObject *(*unary_op)(PyObject*);  /*C89 -- mixed declarations and code*/
...
fold_binop(expr_ty node, PyArena *arena, int optimize)
{
  ...
  { PyObject *lv = get_const_value(lhs);  /*C89 -- mixed declarations and code*/
...
{   int i;
    for (/*int*/ i = 0; i < asdl_seq_LEN(elts); i++) {  /*C89 -- mixed declarations and code*/
...
fold_compare(expr_ty node, PyArena *arena, int optimize)
{
  ...
  { int op = asdl_seq_GET(ops, i);  /*C89 -- mixed declarations and code*/
    if (op == In || op == NotIn) {
...
astfold_body(asdl_seq *stmts, PyArena *ctx_, int optimize_)
{
  ...
  { int docstring = isdocstring((stmt_ty)asdl_seq_GET(stmts, 0));  /*C89 -- mixed declarations and code*/
    CALL_SEQ(astfold_stmt, stmt_ty, stmts);
    if (docstring) {
        return 1;
    }
  }
  { stmt_ty st = (stmt_ty)asdl_seq_GET(stmts, 0);  /*C89 -- mixed declarations and code*/
    if (isdocstring(st)) {
