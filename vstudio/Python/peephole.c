                        ...
                        PyObject *consts, int n)
{
  ...
  { PyObject *newconst = PyTuple_New(n);  /*C89 -- mixed declarations and code*/
    Py_ssize_t i, pos;
    if (newconst == NULL) {
        return -1;
    }
                ...
                PyObject *lnotab_obj)
{
  ...
  { Py_ssize_t codesize = PyBytes_GET_SIZE(code);  /*C89 -- mixed declarations and code*/
  ...
  { Py_ssize_t codelen = codesize / sizeof(_Py_CODEUNIT);
    ...
    for (op_start = i = h = 0; i < codelen; i++, op_start = i) {
      ...
      { Py_ssize_t ilen = i - op_start + 1;  /*C89 -- mixed declarations and code*/
        if (instrsize(j) > ilen) {
            goto exitUnchanged;
        }
        assert(ilen <= INT_MAX);
