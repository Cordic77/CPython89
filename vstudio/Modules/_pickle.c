...
#include "Python.h"
#include "internal/pystate.h"  /*VS9COMPAT: error C2065: '_PyRuntime' : undeclared identifier -- TODO: remove eventually*/
...
enum opcode {
    ...
    POP_MARK        = '1',
    DUP             = '2',
    FLOAT_TYPE      = 'F',  /*VS9COMPAT: conflicts with <WinDef.h> typedefs*/
    INT_TYPE        = 'I',
    BININT          = 'J',
    BININT1         = 'K',
    LONG_TYPE       = 'L',
...
PyMemoTable_Copy(PyMemoTable *self)
{
  ...
  { size_t i;
    for (/*size_t*/ i = 0; i < self->mt_allocated; i++) {  /*C89 -- mixed declarations and code*/
...
PyMemoTable_Set(PyMemoTable *self, PyObject *key, Py_ssize_t value)
{
  ...
  { size_t desired_size = (self->mt_used > 50000 ? 2 : 4) * self->mt_used;  /*C89 -- mixed declarations and code*/
...
_Unpickler_ResizeMemoList(UnpicklerObject *self, size_t new_size)
{
  ...
  { PyObject **memo_new = self->memo;  /*C89 -- mixed declarations and code*/
             ...
             PyObject *global, PyObject *dotted_path)
{
  ...
  { PyObject *candidate = get_deep_attribute(module, dotted_path, NULL);  /*C89 -- mixed declarations and code*/
    if (candidate == NULL) {
        return -1;
    }
    if (candidate != global) {
...
/*[clinic end generated code: output=bb83a919d29225ef input=b73043485ac30b36]*/
{
  ...
  { size_t i;
    for (/*size_t*/ i = 0; i < memo->mt_allocated; ++i) {  /*C89 -- mixed declarations and code*/
...
load_mark(UnpicklerObject *self)
{
    ...
    if ((self->num_marks + 1) >= self->marks_size) {
      ...
      { Py_ssize_t *marks_old = self->marks;  /*C89 -- mixed declarations and code*/
...
#define OP_ARG(opcode, load_func, arg) \
    ...
    while (1) {
        ...
        OP(INT_TYPE, load_int)
        OP(LONG_TYPE, load_long)
        OP_ARG(LONG1, load_counted_long, 1)
        OP_ARG(LONG4, load_counted_long, 4)
        OP(FLOAT_TYPE, load_float)
...
Unpickler_set_memo(UnpicklerObject *self, PyObject *obj, void *Py_UNUSED(ignored))
{
    ...
    if (Py_TYPE(obj) == &UnpicklerMemoProxyType) {
      ...
      { size_t i;
        for (/*size_t*/ i = 0; i < new_memo_size; i++) {  /*C89 -- mixed declarations and code*/
  ...
  error:
    if (new_memo_size) {
      { size_t i;
        for (/*size_t*/ i = new_memo_size - 1; i != SIZE_MAX; i--) {  /*C89 -- mixed declarations and code*/
