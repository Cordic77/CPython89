...
PyCPointerType_from_param(PyObject *type, PyObject *value)
{
    ...
    */
    ...
    if (PointerObject_Check(value) || ArrayObject_Check(value)) {
      ...
      { int ret = PyObject_IsSubclass(v->proto, typedict->proto);  /*C89 -- mixed declarations and code*/
        if (ret < 0) {
            return NULL;
        }
        if (ret) {
...
PyCData_reduce(PyObject *myself, PyObject *args)
{
  ...
  { PyObject *dict = PyObject_GetAttrString(myself, "__dict__");  /*C89 -- mixed declarations and code*/
    if (dict == NULL) {
        return NULL;
    }
