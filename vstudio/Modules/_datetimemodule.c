...
{   size_t i;
    for (/*size_t*/ i = 0; i < num_digits; ++i) {  /*C89 -- mixed declarations and code*/
                  ...
                  int *minute, int *second, int *microsecond)
{
    ...
    // Parse [HH[:MM[:SS]]]
    size_t i;
    for (/*size_t*/ i = 0; i < 3; ++i) {  /*C89 -- mixed declarations and code*/
        p = parse_digits(p, vals[i], 2);
        if (NULL == p) {
            return -3;
        }

      { char c = *(p++);  /*C89 -- mixed declarations and code*/
        if (p >= p_end) {
            return c != '\0';
        }
        else if (c == ':') {
    ...
    // Parse .fff[fff]
  { size_t len_remains = p_end - p;  /*C89 -- mixed declarations and code*/
                     ...
                     int *tzmicrosecond)
{
  ...
  { int rv = parse_hh_mm_ss_ff(dtstr, tzinfo_pos, hour, minute, second,
                               microsecond);  /*C89 -- mixed declarations and code*/
...
delta_repr(PyDateTime_Delta *self)
{
  ...
  { const char *sep = "";  /*C89 -- mixed declarations and code*/
  ...
  { PyObject *repr = PyUnicode_FromFormat("%s(%S)", Py_TYPE(self)->tp_name,  /*C89 -- mixed declarations and code*/
                                          args);
    Py_DECREF(args);
    return repr;
  }
...
_sanitize_isoformat_str(PyObject *dtstr)
{
  ...
  { PyObject *str_out = _PyUnicode_Copy(dtstr);  /*C89 -- mixed declarations and code*/
    if (str_out == NULL) {
        return NULL;
    }
...
datetime_fromisoformat(PyObject *cls, PyObject *dtstr)
{
  ...
  { PyObject *dtstr_clean = _sanitize_isoformat_str(dtstr);  /*C89 -- mixed declarations and code*/
    if (dtstr_clean == NULL) {
        goto error;
    }
