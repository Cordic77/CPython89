...
faulthandler_exc_handler(struct _EXCEPTION_POINTERS *exc_info)
{
    ...
    if (code == EXCEPTION_ACCESS_VIOLATION) {
        ...
        size_t i;
        for (/*size_t*/ i=0; i < faulthandler_nsignals; i++) {  /*C89 -- mixed declarations and code*/
...
faulthandler_init_enable(void)
{
  ...
  { PyObject *res = _PyObject_CallMethodId(module, &PyId_enable, NULL);  /*C89 -- mixed declarations and code*/
    Py_DECREF(module);
    if (res == NULL) {
        return -1;
    }
...
void _PyFaulthandler_Fini(void)
{
    ...
    if (stack.ss_sp != NULL) {
        ...
        stack_t current_stack = {0};  /*C89 -- ISO C forbids empty initializer braces*/
