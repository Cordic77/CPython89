...
{
  if (_abc_data_type.tp_dealloc == NULL)
  {
    _abc_data_type.tp_dealloc = (destructor)abc_data_dealloc;
    _abc_data_type.tp_flags = Py_TPFLAGS_DEFAULT;
    _abc_data_type.tp_alloc = PyType_GenericAlloc;
    _abc_data_type.tp_new = abc_data_new;
  }
}
#endif
...
_in_weak_set(PyObject *set, PyObject *obj)
{
  ...
  { PyObject *ref = PyWeakref_NewRef(obj, NULL);  /*C89 -- mixed declarations and code*/
    if (ref == NULL) {
...
_add_to_weak_set(PyObject **pset, PyObject *obj)
{
  ...
  { PyObject *set = *pset;  /*C89 -- mixed declarations and code*/
    PyObject *ref, *wr;
    PyObject *destroy_cb;
...
compute_abstract_methods(PyObject *self)
{
  ...
  { Py_ssize_t pos;
    for (/*Py_ssize_t*/ pos = 0; pos < PyList_GET_SIZE(items); pos++) {  /*C89 -- mixed declarations and code*/
  ...
  { Py_ssize_t pos;
    for (/*Py_ssize_t*/ pos = 0; pos < PyTuple_GET_SIZE(bases); pos++) {  /*C89 -- mixed declarations and code*/
...
/*[clinic end generated code: output=7851e7668c963524 input=ca589f8c3080e67f]*/
{
  ...
  { int result = PyObject_IsSubclass(subclass, self);  /*C89 -- mixed declarations and code*/
    if (result > 0) {
        Py_INCREF(subclass);
...
/*[clinic end generated code: output=b8b5148f63b6b56f input=a4f4525679261084]*/
{
  ...
  { int incache = _in_weak_set(impl->_abc_cache, subclass);  /*C89 -- mixed declarations and code*/
    if (incache < 0) {
        goto end;
    }
    if (incache > 0) {
...
/*[clinic end generated code: output=b56c9e4a530e3894 input=1d947243409d10b8]*/
{
  ...
  { PyObject *ok, *subclasses = NULL, *result = NULL;  /*C89 -- mixed declarations and code*/
    Py_ssize_t pos;
    int incache;
    _abc_data *impl = _get_impl(self);
    if (impl == NULL) {
  ...
  { PyObject *mro = ((PyTypeObject *)subclass)->tp_mro;  /*C89 -- mixed declarations and code*/
    ...
    for (pos = 0; pos < PyList_GET_SIZE(subclasses); pos++) {
        ...
        Py_INCREF(scls);
      { int r = PyObject_IsSubclass(subclass, scls);  /*C89 -- mixed declarations and code*/
        Py_DECREF(scls);
        if (r > 0) {
            if (_add_to_weak_set(&impl->_abc_cache, subclass) < 0) {
                goto end;
                             ...
                             PyObject **result)
{
  ...
  { Py_ssize_t registry_size = PySet_Size(impl->_abc_registry);  /*C89 -- mixed declarations and code*/
    if (registry_size == 0) {
        return 0;
    }
  ...
  { PyObject *key;
    ...
    for (i = 0; i < registry_size; i++) {
        ...
        Py_INCREF(rkey);
      { int r = PyObject_IsSubclass(subclass, rkey);  /*C89 -- mixed declarations and code*/
        Py_DECREF(rkey);
        if (r < 0) {
            ret = -1;
            break;
