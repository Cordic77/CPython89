...
char _PyIO_get_console_type(PyObject *path_or_fd) {
  ...
  { PyObject *decoded;  /*C89 -- mixed declarations and code*/
...
/*[clinic end generated code: output=3fd9cbcdd8d95429 input=06ae4b863c63244b]*/
{
    ...
    if (fd < 0) {
      ...
      { int d = PyUnicode_FSDecoder(nameobj, (void*)&decodedname);  /*C89 -- mixed declarations and code*/
        if (!d)
            return -1;
      }
...
read_console_w(HANDLE handle, DWORD maxlen, DWORD *readlen) {
    ...
    *readlen = 0;
    ...
    //DebugBreak();
    Py_BEGIN_ALLOW_THREADS
  { DWORD off = 0;  /*C89 -- mixed declarations and code*/
    while (off < maxlen) {
        DWORD n = (DWORD)-1; 
      ...
      { BOOL res = ReadConsoleW(handle, &buf[off], len, &n, NULL);
        ...
        if (n == 0) {
            ...
            if (err != ERROR_OPERATION_ABORTED)
                break;
            err = 0;
          { HANDLE hInterruptEvent = _PyOS_SigintEvent();  /*C89 -- mixed declarations and code*/
      ...
      { WORD char_type;  /*C89 -- mixed declarations and code*/
...
readinto(winconsoleio *self, char *buf, Py_ssize_t len)
{
    ...
    /* Each character may take up to 4 bytes in the final buffer.
       ...
       character.
    */
  { DWORD wlen = (DWORD)(len / 4);  /*C89 -- mixed declarations and code*/
    if (wlen == 0) {
        wlen = 1;
    }
    ...
    while (1) {
        ...
        if (len >= (Py_ssize_t)bufsize) {
          ...
          { wchar_t *tmp = PyMem_Realloc(buf,
                                         (bufsize + 1) * sizeof(wchar_t));  /*C89 -- mixed declarations and code*/
            if (tmp == NULL) {
                PyMem_Free(buf);
                return NULL;
            }
