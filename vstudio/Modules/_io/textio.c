...
_textiowrapper_fix_encoder_state(textio *self)
{
  ...
  { PyObject *cookieObj = PyObject_CallMethodObjArgs(
        self->buffer, _PyIO_str_tell, NULL);  /*C89 -- mixed declarations and code*/
    if (cookieObj == NULL) {
        return -1;
    }
  ...
  { int cmp = PyObject_RichCompareBool(cookieObj, _PyLong_Zero, Py_EQ);
    ...
    if (cmp == 0) {
      ...
      { PyObject *res = PyObject_CallMethodObjArgs(
            self->encoder, _PyIO_str_setstate, _PyLong_Zero, NULL);  /*C89 -- mixed declarations and code*/
        if (res == NULL) {
            return -1;
        }
        Py_DECREF(res);
                              ...
                              PyObject *errors, int newline_changed)
{
  ...
  { const char *c_errors = PyUnicode_AsUTF8(errors);  /*C89 -- mixed declarations and code*/
    if (c_errors == NULL) {
        return -1;
    }
...
/*[clinic end generated code: output=52b812ff4b3d4b0f input=671e82136e0f5822]*/
{
  ...
  { PyObject *res = PyObject_CallMethodObjArgs((PyObject *)self, _PyIO_str_flush, NULL);  /*C89 -- mixed declarations and code*/
    if (res == NULL) {
        return NULL;
    }
    Py_DECREF(res);
...
textiowrapper_read_chunk(textio *self, Py_ssize_t size_hint)
{
    ...
    if (self->telling) {
      ...
      { PyObject *snapshot = Py_BuildValue("NN", dec_flags, next_input);  /*C89 -- mixed declarations and code*/
        if (snapshot == NULL) {
            dec_flags = NULL;
            goto fail;
        }
