...
make_timezones_capi(PyObject *self, PyObject *args) {
  ...
  { PyObject *rv = PyTuple_New(3);  /*C89 -- mixed declarations and code*/
...
#undef CHECK_TSS_API
  ...
  { Py_tss_t *ptr_key = PyThread_tss_alloc();  /*C89 -- mixed declarations and code*/
    if (ptr_key == NULL) {
...
static inline void init_GenericAlias_Type (void)
{
  if (GenericAlias_Type.tp_dealloc == NULL)
  {
    GenericAlias_Type.tp_dealloc = (destructor)generic_alias_dealloc;
    GenericAlias_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE;
    GenericAlias_Type.tp_methods = generic_alias_methods;
  }
}
#endif
...
static inline void init_Generic_Type (void)
{
  if (Generic_Type.tp_dealloc == NULL)
  {
    Generic_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE;
    Generic_Type.tp_methods = generic_methods;
  }
}
#endif
