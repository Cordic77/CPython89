...
#ifdef MS_WINDOWS
  { const wchar_t *var = _wgetenv(wname);  /*C89 -- mixed declarations and code*/
    if (!var || var[0] == '\0') {
        *dest = NULL;
        return 0;
    }
...
pymain_run_startup(PyCompilerFlags *cf)
{
  ...
  { FILE *fp = _Py_fopen(startup, "r");  /*C89 -- mixed declarations and code*/
    if (fp == NULL) {
...
{   int i;
    for (/*int*/ i=0; i < len; i++) {  /*C89 -- mixed declarations and code*/
...
pymain_init_cmdline_argv(_PyMain *pymain, _Py_CommandLineDetails *cmdline)
{
    ...
    if (pymain->use_bytes_argv) {
      ...
      { int i;
        for (/*int*/ i = 0; i < pymain->argc; i++) {  /*C89 -- mixed declarations and code*/
            size_t len;
  ...
  { wchar_t *program;
    if (pymain->argc >= 1 && cmdline->argv != NULL) {  /*C89 -- mixed declarations and code*/
...
pymain_free_raw(_PyMain *pymain)
{
  ...
  { PyMemAllocatorEx old_alloc;  /*C89 -- mixed declarations and code*/
    _PyMem_SetDefaultAllocator(PYMEM_DOMAIN_RAW, &old_alloc);
...
pymain_run_main_from_importer(_PyMain *pymain)
{
  ...
  { int sts = pymain_run_module(L"__main__", 0);  /*C89 -- mixed declarations and code*/
    return (sts != 0);
  }
...
wstrlist_append(int *len, wchar_t ***list, const wchar_t *str)
{
  ...
  { size_t size = (*len + 1) * sizeof(list[0]);  /*C89 -- mixed declarations and code*/
...
config_create_xoptions_dict(const _PyCoreConfig *config)
{
  ...
  { int i;
    for (/*int*/ i=0; i < nxoption; i++) {  /*C89 -- mixed declarations and code*/
...
cmdline_init_env_warnoptions(_Py_CommandLineDetails *cmdline)
{
  ...
  { wchar_t *env;  /*C89 -- mixed declarations and code*/
...
config_init_program_name(_PyCoreConfig *config)
{
  ...
  { const wchar_t *program_name = _Py_path_config.program_name;  /*C89 -- mixed declarations and code*/
  ...
  {
#ifdef MS_WINDOWS
    const wchar_t *default_program_name = L"python";  /*C89 -- mixed declarations and code*/
#else
...
copy_wstrlist(int len, wchar_t **list)
{
  ...
  { size_t size = len * sizeof(list[0]);  /*C89 -- mixed declarations and code*/
...
pymain_init_core_argv(_PyMain *pymain, _Py_CommandLineDetails *cmdline)
{
  ...
  { wchar_t *arg0 = NULL;  /*C89 -- mixed declarations and code*/
...
wstrlist_as_pylist(int len, wchar_t **list)
{
  ...
  { PyObject *pylist = PyList_New(len);  /*C89 -- mixed declarations and code*/
    if (pylist == NULL) {
        return NULL;
    }
...
pymain_import_readline(_PyMain *pymain)
{
  ...
  { PyObject *mod = PyImport_ImportModule("readline");  /*C89 -- mixed declarations and code*/
    if (mod == NULL) {
        PyErr_Clear();
    }
    else {
...
pymain_open_filename(_PyMain *pymain)
{
  ...
  { struct _Py_stat_struct sb;  /*C89 -- mixed declarations and code*/
...
pymain_run_filename(_PyMain *pymain, PyCompilerFlags *cf)
{
  ...
  { FILE *fp;  /*C89 -- mixed declarations and code*/
    if (pymain->filename != NULL) {
...
pymain_repl(_PyMain *pymain, PyCompilerFlags *cf)
{
  ...
  { int res = PyRun_AnyFileFlags(stdin, "<stdin>", cf);  /*C89 -- mixed declarations and code*/
    pymain->status = (res != 0);
  }
}
...
{
    errno = 0;
  { const char *endptr = str;  /*C89 -- mixed declarations and code*/
...
{
    errno = 0;
  { const wchar_t *endptr = wstr;  /*C89 -- mixed declarations and code*/
...
get_env_flag(int *flag, const char *name)
{
  ...
  { int value;  /*C89 -- mixed declarations and code*/
...
config_init_home(_PyCoreConfig *config)
{
  ...
  { int res = config_get_env_var_dup(&home, L"PYTHONHOME", "PYTHONHOME");  /*C89 -- mixed declarations and code*/
    if (res < 0) {
...
config_init_utf8_mode(_PyCoreConfig *config)
{
  ...
  { const wchar_t *xopt = config_get_xoption(config, L"utf8");  /*C89 -- mixed declarations and code*/
    if (xopt) {
  ...
  { const char *opt = config_get_env_var("PYTHONUTF8");  /*C89 -- mixed declarations and code*/
    if (opt) {
        if (strcmp(opt, "1") == 0) {
            config->utf8_mode = 1;
        }
...
config_read_env_vars(_PyCoreConfig *config)
{
  ...
  { const char *env = config_get_env_var("PYTHONCOERCECLOCALE");  /*C89 -- mixed declarations and code*/
    if (env) {
        if (strcmp(env, "0") == 0) {
  ...
  { wchar_t *path;  /*C89 -- mixed declarations and code*/
...
config_read_complex_options(_PyCoreConfig *config)
{
  ...
  { _PyInitError err = pymain_init_tracemalloc(config);  /*C89 -- mixed declarations and code*/
    if (_Py_INIT_FAILED(err)) {
        return err;
    }
  }
...
pymain_read_conf_impl(_PyMain *pymain, _Py_CommandLineDetails *cmdline)
{
  ...
  { _PyCoreConfig *config = &pymain->config;  /*C89 -- mixed declarations and code*/
  ...
  { int init_isolated = Py_IsolatedFlag;  /*C89 -- mixed declarations and code*/
...
pymain_read_conf(_PyMain *pymain, _Py_CommandLineDetails *cmdline)
{
  ...
  { int locale_coerced = 0;  /*C89 -- mixed declarations and code*/
    int loops = 0;
    ...
    while (1) {
      ...
      { int conf_res = pymain_read_conf_impl(pymain, cmdline);  /*C89 -- mixed declarations and code*/
        if (conf_res != 0) {
            res = conf_res;
            goto done;
        }
...
config_init_module_search_paths(_PyCoreConfig *config)
{
  ...
  { const wchar_t *sys_path = Py_GetPath();  /*C89 -- mixed declarations and code*/
    ...
    while (1) {
      ...
      { size_t path_len = (p - sys_path);  /*C89 -- mixed declarations and code*/
...
pymain_init_sys_path(_PyMain *pymain)
{
  ...
  { PyObject *path0;  /*C89 -- mixed declarations and code*/
...
pymain_cmdline_impl(_PyMain *pymain, _Py_CommandLineDetails *cmdline)
{
  ...
  { int res = pymain_read_conf(pymain, cmdline);  /*C89 -- mixed declarations and code*/
    if (res < 0) {
        return -1;
    }
    if (res > 0) {
  ...
  { _PyInitError err = config_init_warnoptions(&pymain->config, cmdline);  /*C89 -- mixed declarations and code*/
    if (_Py_INIT_FAILED(err)) {
        pymain->err = err;
        return -1;
    }
...
pymain_cmdline(_PyMain *pymain)
{
  ...
  {
#ifdef Py_DEBUG
    PyMemAllocatorEx default_alloc;  /*C89 -- mixed declarations and code*/
    PyMem_GetAllocator(PYMEM_DOMAIN_RAW, &default_alloc);
#endif

  { _Py_CommandLineDetails cmdline;  /*C89 -- mixed declarations and code*/
...
pymain_main(_PyMain *pymain)
{
  ...
  { int res = pymain_cmdline(pymain);  /*C89 -- mixed declarations and code*/
    if (res < 0) {
...
Py_Main(int argc, wchar_t **argv)
{
    ...
    #else
    _PyMain pymain = {0};
    pymain.config = _PyCoreConfig_INIT;
    pymain.err = _Py_INIT_OK();
    #endif
