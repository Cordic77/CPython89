                         ...
                         _PyCmdline *cmdline)
{
  ...
  { wchar_t *program;  /*C89 -- mixed declarations and code*/
...
config_init_executable(_PyCoreConfig *config)
{
  ...
  { const wchar_t *program_full_path = _Py_path_config.program_full_path;  /*C89 -- mixed declarations and code*/
...
config_init_tracemalloc(_PyCoreConfig *config)
{
  ...
  { const wchar_t *xoption = config_get_xoption(config, L"tracemalloc");  /*C89 -- mixed declarations and code*/
    if (xoption) {
...
pymain_read_conf(_PyMain *pymain, _PyCoreConfig *config, _PyCmdline *cmdline)
{
  ...
  { int locale_coerced = 0;  /*C89 -- mixed declarations and code*/
    ...
    while (1) {
      ...
      { int conf_res = pymain_read_conf_impl(pymain, config, cmdline);  /*C89 -- mixed declarations and code*/
        if (conf_res != 0) {
            res = conf_res;
            goto done;
        }
      ...
      { int new_utf8_mode = config->utf8_mode;  /*C89 -- mixed declarations and code*/
...
pymain_init_sys_path(_PyMain *pymain, _PyCoreConfig *config)
{
  ...
  { PyObject *path0 = NULL;  /*C89 -- mixed declarations and code*/
                    ...
                    _PyCmdline *cmdline)
{
  ...
  { int res = pymain_read_conf(pymain, config, cmdline);  /*C89 -- mixed declarations and code*/
    if (res < 0) {
        return -1;
    }
    if (res > 0) {
  ...
  { _PyInitError err = config_init_warnoptions(config, cmdline);  /*C89 -- mixed declarations and code*/
    if (_Py_INIT_FAILED(err)) {
        pymain->err = err;
        return -1;
    }
...
pymain_cmdline(_PyMain *pymain, _PyCoreConfig *config)
{
  ...
  { _PyCmdline cmdline;  /*C89 -- mixed declarations and code*/
    memset(&cmdline, 0, sizeof(cmdline));
...
pymain_init(_PyMain *pymain)
{
  ...
  { int res = pymain_cmdline(pymain, config);  /*C89 -- mixed declarations and code*/
    if (res < 0) {
