...
#  ifdef OPENSSL_SYS_WIN32  /*VS9COMPAT: #include "openssl/rand.h" (indirectly) results in #include <WinCrypt.h> which
                                         performs a ‘#define X509_NAME ((LPCSTR) 7)’ that undoes the ‘#undef X509_NAME’
                                         in openssl\kssl.h (which gets included through #include "openssl/ssl.h"). Redo
                                         this #undef to make sure we have no name collissions! */
/*
