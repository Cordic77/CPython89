...
FutureObj_finalize(FutureObj *fut)
{
  ...
  { PyObject *error_type, *error_value, *error_traceback;  /*C89 -- mixed declarations and code*/
...
static inline void init_TaskSendMethWrapper_Type (void)
{
  if (TaskSendMethWrapper_Type.tp_dealloc == NULL)
  {
    TaskSendMethWrapper_Type.tp_basicsize = sizeof(TaskSendMethWrapper);
    TaskSendMethWrapper_Type.tp_itemsize = 0;
    TaskSendMethWrapper_Type.tp_getset = TaskSendMethWrapper_getsetlist;
    TaskSendMethWrapper_Type.tp_dealloc = (destructor)TaskSendMethWrapper_dealloc;
    TaskSendMethWrapper_Type.tp_call = (ternaryfunc)TaskSendMethWrapper_call;
    TaskSendMethWrapper_Type.tp_getattro = PyObject_GenericGetAttr;
    TaskSendMethWrapper_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC;
    TaskSendMethWrapper_Type.tp_traverse = (traverseproc)TaskSendMethWrapper_traverse;
    TaskSendMethWrapper_Type.tp_clear = (inquiry)TaskSendMethWrapper_clear;
  }
}
#endif
...
set_exception:
    ...
    if (o == NULL) {
        ...
        if (o == Py_None) {
            ...
            /* Check if `result` future is attached to a different loop */
          { PyObject *oloop = PyObject_GetAttrString(result, "_loop");  /*C89 -- mixed declarations and code*/
            if (oloop == NULL) {
                goto fail;
            }
...
task_wakeup(TaskObj *task, PyObject *o)
{
  ...
  { PyObject *fut_result = PyObject_CallMethod(o, "result", NULL);  /*C89 -- mixed declarations and code*/
    if (fut_result == NULL) {
        PyObject *et, *ev, *tb;
        PyObject *res;
