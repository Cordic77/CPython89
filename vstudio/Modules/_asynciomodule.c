...
_is_coroutine(PyObject *coro)
{
  ...
  { int is_res_true = PyObject_IsTrue(res);  /*C89 -- mixed declarations and code*/
    Py_DECREF(res);
    if (is_res_true <= 0) {
        return is_res_true;
    }
...
is_coroutine(PyObject *coro)
{
  ...
  { int has_it = PySet_Contains(
        iscoroutine_typecache, (PyObject*) Py_TYPE(coro));  /*C89 -- mixed declarations and code*/
    if (has_it == 0) {
...
get_running_loop(PyObject **loop)
{
  ...
  { PyObject *running_loop = ((PyRunningLoopHolder *)rl)->rl_loop;  /*C89 -- mixed declarations and code*/
...
set_running_loop(PyObject *loop)
{
  ...
  { PyObject *ts_dict = PyThreadState_GetDict();  // borrowed  /*C89 -- mixed declarations and code*/
    if (ts_dict == NULL) {
        PyErr_SetString(
  ...
  { PyRunningLoopHolder *rl = new_running_loop_holder(loop);  /*C89 -- mixed declarations and code*/
    if (rl == NULL) {
        return -1;
    }
...
/*[clinic end generated code: output=f35f940936a4b1e5 input=49ecf9cf5ec50dc5]*/
{
  ...
  { int res = future_get_result(self, &result);  /*C89 -- mixed declarations and code*/
...
FutureObj_get_callbacks(FutureObj *fut, void *Py_UNUSED(ignored))
{
  ...
  { Py_ssize_t len = 1;  /*C89 -- mixed declarations and code*/
    if (fut->fut_callbacks != NULL) {
        len += PyList_GET_SIZE(fut->fut_callbacks);
    }
...
FutureObj_repr(FutureObj *fut)
{
  ...
  { PyObject *rinfo = _PyObject_CallMethodIdObjArgs((PyObject*)fut,
                                                    &PyId__repr_info,
                                                    NULL);  /*C89 -- mixed declarations and code*/
    if (rinfo == NULL) {
        return NULL;
    }
...
static inline void init_FutureType (void)
{
  if (FutureType.tp_dealloc == NULL)
  {
    FutureType.tp_dealloc = FutureObj_dealloc;
    FutureType.tp_as_async = &FutureType_as_async;
    FutureType.tp_repr = (reprfunc)FutureObj_repr;
    FutureType.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC | Py_TPFLAGS_BASETYPE
        | Py_TPFLAGS_HAVE_FINALIZE;
    FutureType.tp_doc = _asyncio_Future___init____doc__;
    FutureType.tp_traverse = (traverseproc)FutureObj_traverse;
    FutureType.tp_clear = (inquiry)FutureObj_clear;
    FutureType.tp_weaklistoffset = offsetof(FutureObj, fut_weakreflist);
    FutureType.tp_iter = (getiterfunc)future_new_iter;
    FutureType.tp_methods = FutureType_methods;
    FutureType.tp_getset = FutureType_getsetlist;
    FutureType.tp_dictoffset = offsetof(FutureObj, dict);
    FutureType.tp_init = (initproc)_asyncio_Future___init__;
    FutureType.tp_new = PyType_GenericNew;
    FutureType.tp_finalize = (destructor)FutureObj_finalize;
  }
}
#endif
...
static inline void init_FutureIterType (void)
{
  if (FutureIterType.tp_dealloc == NULL)
  {
    FutureIterType.tp_basicsize = sizeof(futureiterobject);
    FutureIterType.tp_itemsize = 0;
    FutureIterType.tp_dealloc = (destructor)FutureIter_dealloc;
    FutureIterType.tp_getattro = PyObject_GenericGetAttr;
    FutureIterType.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC;
    FutureIterType.tp_traverse = (traverseproc)FutureIter_traverse;
    FutureIterType.tp_iter = PyObject_SelfIter;
    FutureIterType.tp_iternext = (iternextfunc)FutureIter_iternext;
    FutureIterType.tp_methods = FutureIter_methods;
  }
}
#endif
...
static inline void init_TaskStepMethWrapper_Type (void)
{
  if (TaskStepMethWrapper_Type.tp_dealloc == NULL)
  {
    TaskStepMethWrapper_Type.tp_basicsize = sizeof(TaskStepMethWrapper);
    TaskStepMethWrapper_Type.tp_itemsize = 0;
    TaskStepMethWrapper_Type.tp_getset = TaskStepMethWrapper_getsetlist;
    TaskStepMethWrapper_Type.tp_dealloc = (destructor)TaskStepMethWrapper_dealloc;
    TaskStepMethWrapper_Type.tp_call = (ternaryfunc)TaskStepMethWrapper_call;
    TaskStepMethWrapper_Type.tp_getattro = PyObject_GenericGetAttr;
    TaskStepMethWrapper_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC;
    TaskStepMethWrapper_Type.tp_traverse = (traverseproc)TaskStepMethWrapper_traverse;
    TaskStepMethWrapper_Type.tp_clear = (inquiry)TaskStepMethWrapper_clear;
  }
}
#endif
...
static inline void init_TaskWakeupMethWrapper_Type (void)
{
  if (TaskWakeupMethWrapper_Type.tp_dealloc == NULL)
  {
    TaskWakeupMethWrapper_Type.tp_basicsize = sizeof(TaskWakeupMethWrapper);
    TaskWakeupMethWrapper_Type.tp_itemsize = 0;
    TaskWakeupMethWrapper_Type.tp_dealloc = (destructor)TaskWakeupMethWrapper_dealloc;
    TaskWakeupMethWrapper_Type.tp_call = (ternaryfunc)TaskWakeupMethWrapper_call;
    TaskWakeupMethWrapper_Type.tp_getattro = PyObject_GenericGetAttr;
    TaskWakeupMethWrapper_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC;
    TaskWakeupMethWrapper_Type.tp_traverse = (traverseproc)TaskWakeupMethWrapper_traverse;
    TaskWakeupMethWrapper_Type.tp_clear = (inquiry)TaskWakeupMethWrapper_clear;
  }
}
#endif
...
/*[clinic end generated code: output=9f24774c2287fc2f input=8d132974b049593e]*/
{
  ...
  { int is_coro = is_coroutine(coro);  /*C89 -- mixed declarations and code*/
    if (is_coro == -1) {
        return -1;
    }
    if (is_coro == 0) {
...
static inline void init_TaskType (void)
{
  if (TaskType.tp_dealloc == NULL)
  {
    TaskType.tp_base = &FutureType;
    TaskType.tp_dealloc = TaskObj_dealloc;
    TaskType.tp_as_async = &FutureType_as_async;
    TaskType.tp_repr = (reprfunc)FutureObj_repr;
    TaskType.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC | Py_TPFLAGS_BASETYPE
        | Py_TPFLAGS_HAVE_FINALIZE;
    TaskType.tp_doc = _asyncio_Task___init____doc__;
    TaskType.tp_traverse = (traverseproc)TaskObj_traverse;
    TaskType.tp_clear = (inquiry)TaskObj_clear;
    TaskType.tp_weaklistoffset = offsetof(TaskObj, task_weakreflist);
    TaskType.tp_iter = (getiterfunc)future_new_iter;
    TaskType.tp_methods = TaskType_methods;
    TaskType.tp_getset = TaskType_getsetlist;
    TaskType.tp_dictoffset = offsetof(TaskObj, dict);
    TaskType.tp_init = (initproc)_asyncio_Task___init__;
    TaskType.tp_new = PyType_GenericNew;
    TaskType.tp_finalize = (destructor)TaskObj_finalize;
  }
}
#endif
...
task_call_step_soon(TaskObj *task, PyObject *arg)
{
  ...
  { int ret = call_soon(task->task_loop, cb, NULL, task->task_context);  /*C89 -- mixed declarations and code*/
    Py_DECREF(cb);
    return ret;
  }
}
...
task_set_error_soon(TaskObj *task, PyObject *et, const char *format, ...)
{
  ...
  { PyObject *e = PyObject_CallFunctionObjArgs(et, msg, NULL);  /*C89 -- mixed declarations and code*/
    Py_DECREF(msg);
    if (e == NULL) {
        return NULL;
    }
...
task_step_impl(TaskObj *task, PyObject *exc)
{
    ...
    if (result == NULL) {
        ...
        if (_PyGen_FetchStopIterationValue(&o) == 0) {
          ...
          { PyObject *res = future_set_result((FutureObj*)task, o);  /*C89 -- mixed declarations and code*/
            Py_DECREF(o);
            if (res == NULL) {
                return NULL;
            }
...
set_exception:
    ...
    if (o == NULL) {
        ...
        if (o == Py_None) {
            ...
            /* Check if `result` future is attached to a different loop */
          { PyObject *oloop = get_future_loop(result);  /*C89 -- mixed declarations and code*/
            if (oloop == NULL) {
                goto fail;
            }
            ...
            if (blocking) {
              ...
              { PyObject *add_cb = _PyObject_GetAttrId(
                    result, &PyId_add_done_callback);  /*C89 -- mixed declarations and code*/
                if (add_cb == NULL) {
                    Py_DECREF(wrapper);
                    goto fail;
                }
              { PyObject *stack[2];  /*C89 -- mixed declarations and code*/
                stack[0] = wrapper;
...
static inline void init_PyRunningLoopHolder_Type (void)
{
  if (PyRunningLoopHolder_Type.tp_dealloc == NULL)
  {
    PyRunningLoopHolder_Type.tp_getattro = PyObject_GenericGetAttr;
    PyRunningLoopHolder_Type.tp_flags = Py_TPFLAGS_DEFAULT;
    PyRunningLoopHolder_Type.tp_dealloc = (destructor)PyRunningLoopHolder_tp_dealloc;
  }
}
#endif
...
module_init(void)
{
  ...
  { PyObject *context_str = PyUnicode_FromString("context");  /*C89 -- mixed declarations and code*/
    if (context_str == NULL) {
        goto fail;
    }
...
#define GET_MOD_ATTR(VAR, NAME) \
  ...
  { PyObject *weak_set;  /*C89 -- mixed declarations and code*/
    WITH_MOD("weakref")
...
PyInit__asyncio(void)
{
    ...
    #if defined(ISO_C99) && (ISO_C99 == 0)  /*C89 -- designated initializers aren't supported*/
    init_TaskStepMethWrapper_Type();
    #endif
    if (PyType_Ready(&TaskStepMethWrapper_Type) < 0) {
        return NULL;
    }
    #if defined(ISO_C99) && (ISO_C99 == 0)  /*C89 -- designated initializers aren't supported*/
    init_TaskWakeupMethWrapper_Type();
    #endif
    ...
    #if defined(ISO_C99) && (ISO_C99 == 0)  /*C89 -- designated initializers aren't supported*/
    init_PyRunningLoopHolder_Type();
    #endif
    if (PyType_Ready(&PyRunningLoopHolder_Type) < 0) {
        return NULL;
    }

  { PyObject *m = PyModule_Create(&_asynciomodule);  /*C89 -- mixed declarations and code*/
    if (m == NULL) {
        return NULL;
    }
