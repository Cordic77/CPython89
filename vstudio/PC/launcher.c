...
{   const char *s;
    for (/*const char **/ s = strstr(buffer, "home"); s; s = strstr(s + 1, "\nhome")) {  /*C89 -- mixed declarations and code*/
        if (*s == '\n') {
            ++s;
        }
      { int i;
        for (/*int*/ i = 4; i > 0 && *s; --i, ++s);  /*C89 -- mixed declarations and code*/
      }
...
wcsdup_pad(const wchar_t *s, int padding, int *newlen)
{
    size_t len = wcslen(s);
    len += 1 + padding;
  { wchar_t *r = (wchar_t *)malloc(len * sizeof(wchar_t));  /*C89 -- mixed declarations and code*/
    if (!r) {
        return NULL;
    }
    if (wcscpy_s(r, len, s)) {
