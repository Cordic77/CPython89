...
#ifdef Py_ENABLE_SHARED
  { extern HANDLE PyWin_DLLhModule;  /*C89 -- mixed declarations and code*/
    if (PyWin_DLLhModule) {
              ...
              int *isolated, int *nosite)
{
  ...
  { size_t bufsiz = MAXPATHLEN;  /*C89 -- mixed declarations and code*/
    ...
    while (!feof(sp_file)) {
      ...
      { DWORD wn = MultiByteToWideChar(CP_UTF8, 0, line, -1, NULL, 0);  /*C89 -- mixed declarations and code*/
...
calculate_pyvenv_file(PyCalculatePath *calculate)
{
  ...
  { FILE *env_file = _Py_wfopen(envbuffer, L"r");  /*C89 -- mixed declarations and code*/
    if (env_file == NULL) {
        errno = 0;
  ...
  { wchar_t tmpbuffer[MAXPATHLEN+1];  /*C89 -- mixed declarations and code*/
                             ...
                             wchar_t *prefix)
{
  ...
  { int skipdefault = (core_config->module_search_path_env != NULL ||
                       ...
                       calculate->user_path != NULL);  /*C89 -- mixed declarations and code*/
...
_PyPathConfig_Calculate(_PyPathConfig *config, const _PyCoreConfig *core_config)
{
  ...
  { _PyInitError err = calculate_path_impl(core_config, &calculate, config);  /*C89 -- mixed declarations and code*/
    if (_Py_INIT_FAILED(err)) {
        goto done;
    }
